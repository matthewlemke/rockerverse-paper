...
[CyVerse](https://cyverse.org) is a NSF supported cyberinfrastructure for the life sciences that enables data driven discovery \citep{merchant2016iplant}. CyVerse supports a 'data science workbench', called the [Discovery Environment (DE)](https://cyverse-de.github.io/). The DE hosts Docker containers that users integrate from the DockerHub or other Docker registries. The DE uses a combination of [HTCondor](https://research.cs.wisc.edu/htcondor/) & Kubernetes for managing its resource pool, it also uses an [OAuth](https://oauth.net/2/) system with [LDAP](https://ldap.com/) for 70+ thousand user accounts in an HTTPS configuration. Because the DE requires HTTPS, a reverse proxy for security is required to use RStudio. For the reverse proxy, containers require `nginx` to handle the communications with OAUTH. CyVerse multi-petabyte data store uses [iRODS](https://irods.org/), an open source data management system. The Rocker Project's RStudio and Shiny containers have been integrated into the DE by building new container layers from the official images. CyVerse maintains its own [GitHub Organization](https://github.com/cyverse-vice/) with Dockerfiles and configuration scripts which are auto-built on the DockerHub. As a final measure for maintaining reproducibility, CyVerse operates its own private Docker registry, where published (publically available) containers are held. 
