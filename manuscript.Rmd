...
and Kubernetes for managing its resource pool. The DE uses an [OAuth](https://oauth.net/2/) system with [LDAP](https://ldap.com/) for 70+ thousand user accounts in an HTTPS
configuration, and is linked via APIs to NSF-XSEDE computing. Rocker integration
in the DE is designed to provide life science researchers with scalable,
compute-intensive R analysis capabilities needed for large and complex datasets
(e.g. genomics/multi-omics, GWAS, phenotypic data, etc.). The DE runs Docker
...
flexibility similar to Binder, but allows containers to run on larger resources
(4-16 cores, 2-64 GB RAM), for longer periods of time (days to weeks).
In addition, CyVerse maintains a multi-petabyte [iRODS](https://irods.org/) Data
Store. This allows users to work with hundreds of GB of data in a DE Rocker
container, maintain it persistently as needed in the Data Store, and move data
between and to other systems (e.g. NSF-XSEDE) or databases (e.g NCBI). The
...
`nginx` to handle communication with DE's HTTPS/OAUTH configuration
\citep{rstudio_proxy_2020}). CyVerse maintains its own [GitHub Organization](https://github.com/cyverse-vice/) with Dockerfiles and configuration scripts auto-built on
