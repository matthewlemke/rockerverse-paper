...
[CyVerse](https://cyverse.org) is a NSF supported cyberinfrastructure for the life sciences that enables data driven discovery \citep{merchant2016iplant}. 
CyVerse supports a browser-based 'data science workbench', called the [Discovery Environment (DE)](https://cyverse-de.github.io/) which uses a combination of [HTCondor](https://research.cs.wisc.edu/htcondor/) & Kubernetes for managing its resource pool, it also uses an [OAuth](https://oauth.net/2/) system with [LDAP](https://ldap.com/) for 70+ thousand user accounts in an HTTPS configuration. 
The DE runs Docker containers on demand that users integrate from the DockerHub or other Docker registries \citep{devisetty2016bringing}. 
Because the DE requires HTTPS, a reverse proxy for security is required to use RStudio. For the reverse proxy, RStudio containers require `nginx` to handle communication with OAUTH \citep{rstudio_proxy_2020}. 
The Rocker Project's RStudio and Shiny containers are integrated into the DE by building new layers from official Rocker images. 
CyVerse maintains its own [GitHub Organization](https://github.com/cyverse-vice/) with Dockerfiles and configuration scripts which are auto-built on the DockerHub. 
As a final measure toward maintaining reproducibility, CyVerse operates its own private Docker registry where user's published (publically available) containers are held. 
CyVerse DE is a similar concept to Binder, but containers can be run on larger resources (4-16 cores, 2-64 GB RAM), for longer periods of time (days to weeks). 
Additionally, CyVerse maintains multi-petabyte [iRODS](https://irods.org/) data store. Users working in DE RStudio containers can bring hundreds of GB of data from the internet or the data store into their running containers, and write their analyses back to their personal spaces in the data store.  
